function A = TuringSimulationSnapShot(systemSize, parameters, fileNameBase)

% take a snapshot at every tt seconds
...
%     B = reshape(M(2,:), systemSize, systemSize);
    ...
    dx = 0.00001;
        ...
        imwrite(img, [fileNameBase, '0.png'],'png');
    ...
    for t=0:dt:T
      ...
      %  fb = max(max(abs(B - Bp)))/mb; % check the maximum
        ...
        if(rem(counter, 500) == 0)
            ...
            imwrite(img, [fileNameBase, num2str(counter*dt), '.png'],'png');
