...
imageFolder = 'Z:\Andre\2021 in vitro outcomes\LBC2GJA1set_2';
load('testBETSEGJMosaic5050Autoscale3formulamatfile.mat', 'ans');
formula = ans;
color = 1;
outputFile = 'Z:\Andre\Code\TuringPatterns\298thtestoutput';
outputFile2 = 'Z:\Andre\Code\TuringPatterns\298thBtestoutput33.xlsx';
%imageFolder2 = [imageFolder, 'Ex14BETSE'];
%mkdir(imageFolder2);
makePlot = true;
...
for ii=1:len
    ...
    data = min(data, [], 3); %new CP
    data= data < 200; %new CP
    data=bwareafilt(data, 1); %new CP
    [rows, columns] = find(data); %new CP
    row1 = min(rows); %new CP
    row2 = max(rows); %new CP
    col1 = min(columns); %new CP
    col2 = max(columns); %new CP
    data=data(row1:row2, col1:col2, :); %new CP
