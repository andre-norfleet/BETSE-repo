...
for i=1:length(imageNames)
    if(length( imageNames(i).name ) >= 2 && imageNames(i).name(1) ~= 'R' && imageNames(i).name(end) == 'g' && imageNames(i).name(end-1) == 'n')
        imCount = imCount + 1;
...
OUTPUT = uint8(zeros(patternSize*imCount*5/4 , patternSize + TreeSize(2) + sp, 3));
...
for i=1:imCount
    ...
    ss = 1 + int16((i-1)*patternSize*5/4);
    si = ss;
...
imwrite(OUTPUT, [folderName, 'RAll', '.png'],'png');
